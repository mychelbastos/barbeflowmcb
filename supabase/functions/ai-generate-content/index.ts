...
{"title": "...", "description": "..."}`;
      ...
      const imagePrompts: Record<AllowedTable, string> = {
        products: `Professional product photography of "${item.name}"${item.description ? ` (${item.description})` : ''}. Dark moody background, warm amber lighting, studio quality, centered, luxurious e-commerce aesthetic. Sharp focus, high resolution. IMPORTANT: Do NOT include any text, words, letters, watermarks, logos or typography in the image. Pure photography only.`,
        services: `Realistic close-up photograph of a ${genderSubject} client in a ${environmentDesc} receiving the service: "${item.name}"${item.description ? ` (${item.description})` : ''}. The image must show the specific procedure being performed on ${genderNoun}. ${isFeminine ? 'Feature a feminine subject. Elegant beauty salon environment' : 'Always feature masculine subjects. Professional barbershop environment'}, warm cinematic lighting, shallow depth of field, editorial photography style. IMPORTANT: Do NOT include any text, words, letters, watermarks, logos or typography in the image. Pure photography only.`,
        service_packages: `Elegant promotional image for a service package called "${item.name}"${item.description ? ` (${item.description})` : ''}. Show multiple ${isFeminine ? 'beauty salon' : 'barbershop'} services being performed on ${isFeminine ? 'female' : 'male'} clients. Premium warm tones, luxury ${isFeminine ? 'feminine' : 'masculine'} atmosphere, professional marketing style. IMPORTANT: Do NOT include any text, words, letters, watermarks, logos or typography in the image. Pure photography only.`,
        subscription_plans: `Realistic close-up photograph of a ${genderSubject} client in a ${environmentDesc} receiving a premium grooming service. ${isFeminine ? 'Feature a feminine subject in an elegant beauty salon' : 'Feature a masculine subject in a professional barbershop'}. Warm cinematic lighting, shallow depth of field, editorial photography style. At the bottom of the image, include a subtle dark gradient overlay with the exact text "${item.name}" written in elegant white or gold typography. ONLY include that exact name, nothing else. The text should be clean, legible and professionally styled like a premium banner.`,
      };
