...
DROP FUNCTION IF EXISTS public.record_local_payment_for_booking(uuid,uuid,uuid,uuid,jsonb,boolean,uuid,uuid,jsonb);
...
BEGIN
  ...
  -- =====================================================
  -- 6b. Record debt settlement in cash register
  --     When customer has negative balance (owes money),
  --     the extra payment portion must appear in cash.
  -- =====================================================
  IF v_balance < 0 THEN
    v_debt_settlement := LEAST(ABS(v_balance), GREATEST(0, v_total_received - v_total_unpaid));
    
    IF v_debt_settlement > 0 THEN
      INSERT INTO cash_entries (
        tenant_id, session_id, amount_cents, kind, source,
        payment_method, booking_id, notes, occurred_at, staff_id
      ) VALUES (
        p_tenant_id, p_cash_session_id, v_debt_settlement,
        'income', 'booking',
        v_pay_method, p_booking_id,
        'Quitação saldo devedor cliente',
        now(), p_staff_id
      );
    END IF;
  END IF;
