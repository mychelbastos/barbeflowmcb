...
import { LayoutDashboard, Store, TrendingUp, ListChecks, CreditCard, ArrowLeft } from "lucide-react";
...
const navItems = [
  ...
  { to: "/admin/onboarding", icon: ListChecks, label: "Onboarding" },
            ...
            <NavLink key={item.to} to={item.to} end={item.end}
              className={({ isActive }) =>
                `flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm transition-colors ${
                  isActive ? "bg-[hsl(44,65%,54%)]/10 text-[hsl(44,65%,54%)] font-medium"
